...
namespace Microsoft.Extensions.Caching.SqlServerInMemory
{
    ...
    internal static class MonoSqlParameterCollectionExtensions
    {
        public static SqlParameterCollection AddExpiresAtTimeMono(
            ...
            DateTime utcTime)
        {
            return parameters.AddWithValue(Columns.Names.ExpiresAtTime, SqlDbType.DateTime, utcTime);
        }
                    ...
                    DateTime? utcTime)
        {
            if (utcTime.HasValue)
            {
                return parameters.AddWithValue(
                    Columns.Names.AbsoluteExpiration, SqlDbType.DateTime, utcTime.Value);
            }
            else
            {
                return parameters.AddWithValue(
